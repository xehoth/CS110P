...
#pragma omp parallel
  {
    ...
    for (int i = omp_get_thread_num(); i < ARRAY_SIZE; i += ths) {
      z[i] = x[i] + y[i];
    }
  }
}
...
#pragma omp parallel
  { 
    ...
    block = ARRAY_SIZE / ths;
    const int start = id * block;
    for (int i = start; i < start + block && i < ARRAY_SIZE; i++) z[i] = x[i] + y[i];
  }
  for (int i = ths * block; i < ARRAY_SIZE; ++i) z[i] = x[i] + y[i];
}
